# CATALOG — full per-file catalog

BRIEF: Cold, on-demand catalog of individual files with one-line briefs. Read this
when you need to locate a specific file. The hot navigation map (HOT/WARM/COLD folder
tables) lives in `INDEX.md` — go there first; come here for per-file detail.

Navigation map → `INDEX.md`

## WARM — per-file detail (folders are listed in INDEX.md)
